...
var data = featureService.map(function(feature) {
	...
	function setAxisVal(areanm, areanmhc, areacd) {
		d3.select("#keyvalue")
			...
			.html(function() {
				console.log(dataProp[areacd]);
				if(parseInt(d3.select("body").style("width")) <= 600) {
					if (!isNaN(dataAll[areacd])) {
						
						return areanmhc + "<div><span class='msoacodetext'>MSOA " + areanm + "</span></div><div id='seconddiv'>" + dataAll[areacd] + " care home resident deaths involving Covid-19<span class='deathtext'><br>" + dataProp[areacd] + "% of all Covid-19 deaths in area</span></div>";
					} else {
						return areanmhc + "<div><span class='msoacodetext'>MSOA " + areanm + "</span></div><div id='seconddiv'>" + dataAll[areacd] + " care home resident deaths involving Covid-19<span class='deathtext'><br>" + dataProp[areacd] + "% of all Covid-19 deaths in area</span></div>";
					}
				} else {
		...
		d3.select("#legendVal1").text(dataProp[areacd]);


		d3.select("#legendx0").style("width", dataAll[areacd] + "px");
		d3.select("#legendx1").style("width", dataProp[areacd] + "px");
